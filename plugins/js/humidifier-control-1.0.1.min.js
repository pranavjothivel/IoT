...
var status_control_url = "https://api.particle.io/v1/devices/"+deviceID+"/status";
...
function CurrentStatusCallback(data, status) {
    if (status == "success") {
        ...
        setInterval(CurrentHumidityRetriever, 1000);
...
function CurrentHumidityCallback(data, status) {
    if (status == "success") {
        ...
        document.getElementById("AlarmStatus").innerHTML=humidity;
        // create area for humidity
        
        setInterval(CurrentStatusRetriever, 1000);
    }
}
...
function CurrentRunPointCallback(data, status) {
    if (status == "success") {
        ...
        setInterval(CurrentRunPoint, 1000);
...
function sendRunPoint() {
    message = $("#RunPointValue").val();
    $.post(active_control_url, {params: message, access_token: accessToken}, ActiveControlCallback());
